...
void loop() 
{
  ...
  { 
    ...
    if(data == 6)
    {
      //End run and move back to start position
      column = 0;
      row = 0;
      MoveTo(0,250);
      state = 0;
...
//Function to move to the next fraction
void nextFraction()
{
    float xOffset = 14.3;
    float yOffset = 11.2;
...
void CollectFractions96WellPlate(float outletX, float outletY, int timeBetweenFractions)
{
  ...
  for(int i = 0; i < 8; ++i)
  {
    for(int j = 0; j<12; ++j)
    {
      ...
      targetX = outletX + plateWidth/2 - xOffset + j*distanceBetweenWells; //changed to - plateWidt
      targetY = outletY - plateHeight/2 + yOffset - i*distanceBetweenWells; //changed to + i*distance
...
{ 
  ...
  {
    //First calibrate the upper arm - each iteration of the loop, read the state of the buttons
    button1 = digitalRead(button1Pin);
    button2 = digitalRead(button2Pin);
    
    //While the first button has not been pressed, move the stepper in one direction  
    while(!button1PreviouslyPressed)
    {    
      TurnStepper(2,false,1,1);
      
      //Count how many steps required to reach the button
      CWsteps += 1;
      
      //Keep track of the button state and if pressed set the pressed variable to true to escape this loop
      button1 = digitalRead(button1Pin);
      if(button1)
      {
    ...
    //Repeat the calibration going the other direction
    while(!button2PreviouslyPressed)
    {    
      TurnStepper(2,true,1,1);
      CCWsteps += 1;
      button2 = digitalRead(button2Pin);
      if(button2)
      {
    ...
    //When both buttons have been pressed, calculate how many steps to move back to reach the middle and move to the middle
    if(button1PreviouslyPressed && button2PreviouslyPressed)
    {
      totalSteps = CCWsteps/2;
      
      //Move to the middle
      TurnStepper(2,false,totalSteps,1);
      
      //Set the buttons back to false so can use them for further error catching
      button1PreviouslyPressed =false;
      button2PreviouslyPressed = false;
    }
    
    //Calibration of the upper arm finished
    int upperArmRange = totalSteps*2;
    
    //Move the upper arm to one corner
   TurnStepper(2,false,(upperArmRange/3),1);

    //Calibrate the lower arm
    ...
    //While the first button (3) has not been pressed, move the stepper in one direction  
    while(!button3PreviouslyPressed)
    {    
      TurnStepper(1,false,1,1);
   ...
   //Move the lower arm back to start
   TurnStepper(1,true,CWsteps,1);   
   
   //Move the upper arm to the other extreme
   TurnStepper(2,true,(2*upperArmRange/3),1);
   
    //Repeat the calibration going the other direction
    while(!button4PreviouslyPressed)
    {    
      TurnStepper(1,true,1,1);
      CCWsteps += 1;
      button4 = digitalRead(button4Pin);
      if(button4)
      {
    ...
    //When both buttons have been pressed, calculate how many steps to move back to reach the middle and move to the middle
    if(button3PreviouslyPressed && button4PreviouslyPressed)
    {
      totalSteps = (CCWsteps + CWsteps)/2;
      
      //Move lower arm back to the middle
      TurnStepper(1,false,totalSteps,1);
    ...
    //Move the upper arm back to the middle
    TurnStepper(2, false, float(upperArmRange/3),1);
