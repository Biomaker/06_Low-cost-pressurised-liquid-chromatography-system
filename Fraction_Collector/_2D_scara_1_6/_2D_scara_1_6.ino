...
void loop() 
{
  ...
  while(state == 1)
  {
    Serial.println("Reached next state");
  }
}
...
int CalibrateArmPosition()
{
  ...
  while(!calibrationFinished) 
  {
    ...
    if(button1PreviouslyPressed && button2PreviouslyPressed)
    {
      ...
      TurnStepper(2,false,totalSteps,3);
    ...
    //Calibration of the upper arm
    int upperArmRange = int(totalSteps);
    
    //Move the upper arm to one corner
   TurnStepper(2,false,upperArmRange3);
   ...
   TurnStepper(2,true,upperArmRange*2,3);
    ...
    {
      float totalSteps = float(CCWsteps)/2;
      TurnStepper(1,false,totalSteps,3);
    ...
    TurnStepper(2, false, upperArmRange,3);
